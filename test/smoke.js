...
(async () => {
  ...
  step('reject −2; lifelines; accept-open/cancel keeps remaining; accept +2');
  r = await post('/api/action/reject', { token: A.token });
  assert.strictEqual(r.data.team.score, -2);
  ...
  assert.strictEqual(r.data.team.score, 0);
  ...
  assert.strictEqual(r.data.leaderboard.find((t) => t.id === A.teamId).score, 4);
  ...
  assert.strictEqual(r.data.phase, 'finished');
  assert.strictEqual(r.data.leaderboard.find((t) => t.id === T1.teamId).score, 2 + 2 + 5);
  assert.strictEqual(r.data.leaderboard.find((t) => t.id === T2.teamId).score, 2);
