...
(async () => {
  ...
  check('score is 3 after accept', await player.waitFor(`${textOf('[data-bind="score"]')} === '3'`));
  check('score float shows +3', await player.waitFor(`${textOf('[data-bind="score-float"]')} === '+3' && ${visible('[data-bind="score-float"]')}`, 3000));
  ...
  check('score is 2 after reject', await player.waitFor(`${textOf('[data-bind="score"]')} === '2'`));
  ...
  check('projection row for the team shows delta and score', await screen.waitFor(`(() => { const row = [...document.querySelectorAll('[data-board] [data-team]')].find(r => ((r.querySelector('.row-name') || {}).textContent || '').includes(${JSON.stringify(TEAM_A)})); if (!row) return false; const d = ((row.querySelector('.row-delta') || {}).textContent || '').trim(); const s = ((row.querySelector('.row-score') || {}).textContent || '').trim(); return /[+\\-−]\\d/.test(d) && s === '2'; })()`, 4000));
  ...
  check('reload → straight back to board with same score (no onboarding)', await player.waitFor(`${screenIs('board')} && ${textOf('[data-bind="score"]')} === '2'`));
  ...
  check('player score is 7 after scoring', await player.waitFor(`${textOf('[data-bind="score"]')} === '7'`));
  ...
  check('player final score 7', await player.waitFor(`${textOf('[data-bind="final-score"]')} === '7'`));
  ...
  check('podium p1 score 7', await screen.waitFor(`${textOf('[data-bind="p1-score"]')} === '7'`));
